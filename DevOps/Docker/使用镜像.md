...
另外，`docker image ls`列出的镜像体积总和并非是所有镜像十几硬盘消耗。由于 Docker 镜像是分层存储结构，并且可以继承、复用，因此不同镜像可能会使用相同的基础镜像，从而拥有共同的层。由于 Docker 使用 **Union FS**，相同层只需要存储一份即可，因此实际镜像硬盘占用空间很可能要比这个列表镜像大小的总和小得多。
...
`ENTRYPOINT`的格式和`RUN`指令格式一下，分为`EXEC`格式和`shell`格式。
